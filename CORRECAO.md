# Correção TicketPrime (alphabit) — AV1
...
| 5 | `/src` com arquivos `.cs` contendo `app.MapGet` ou `app.MapPost` | ✅ 1 | `src/TicketPrime.API/Program.cs` registra dezenas de rotas com `app.MapGet`, `app.MapPost`, `app.MapPut` e `app.MapDelete` (ex: `/api/usuarios`, `/api/eventos`, `/api/reservas`). |
...
| 9 | `/tests` com `.cs` contendo `[Fact]` ou `[Theory]` | ✅ 1 | `tests/TicketPrime.Tests/TicketPrimeRulesTests.cs`, `TicketPrimeRiskTests.cs` e `UnitTest1.cs` contêm várias anotações `[Fact]`, provando que o xUnit está configurado. |
...
O projeto **alphabit / TicketPrime** atinge nota cheia na AV1 demonstrando forte aderência aos fundamentos de engenharia de software cobrados:
